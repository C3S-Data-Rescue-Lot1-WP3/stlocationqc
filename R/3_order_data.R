#' 
#' Concatenate all the files with country names and/or marine region names
...
order_data <- function() {
  ...
  if (file.exists("countries.txt")) {
    ...
    #colnames(df1) <- c("id", "longitude", "latitude", "geo_name")
    # Concatenate the data.frames by row
    df <- rbind(df, df1)
